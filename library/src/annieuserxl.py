...
def readdata(filename, verbose=0):
  ...
  if 'phonenumber' in df: df['phonenumber'].str.strip()
  if 'superuser' in df: df['superuser'].str.strip()
  if 'notifications' in df: df['notifications'].str.strip()
  return df
...
def writedata(df, countrycode, verbose):
  ...
  for index, row in df.iterrows():
    ...
    dataok = True
    if (
      (not "userid" in r or not r["userid"]) and
      (not "email" in r or not r["email"]) and
      (not "firstname" in r or not r["firstname"]) and
      (not "lastname" in r or not r["lastname"]) and
      (not "phonenumber" in r or not r["phonenumber"])
      ):
      dataok = False
    if dataok:
      # replace "'" to "´"
      for k, c in r.items():
        if c and type(c) is str:
          r[k] = c.replace("'", "´")
      # fix superuser to 0/1 (boolean)
      if "superuser" in r and r["superuser"]:
        if r["superuser"].lower() == "true":
          r["superuser"] = 1
        elif not isinstance(r["superuser"], int):
          r["superuser"] = 0
