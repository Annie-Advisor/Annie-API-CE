...
switch ($method) {
  case 'GET':
    ...
    $result = (object)array(
      "description" => "Check for anniuser having no proper name. Ids contains annieuser.id values.",
      "problem_count" => 0,
      "count" => 0,
      "ids" => array(),
    );
    ...
    foreach ($rows as $rownum => $row) {
      ...
      // check meta for warning
      if (!isset($annieusermeta)) {
        array_push($result->ids, $id);
        $problem_count++;
      } else {
        if (!array_key_exists('firstname', $annieusermeta)) {
          array_push($result->ids, $id);
          $problem_count++;
        } else //to-do-ish: else or just another if (for head count else)?
        if (!array_key_exists('lastname', $annieusermeta)) {
          array_push($result->ids, $id);
          $problem_count++;
        } else
        if (empty($annieusermeta->firstname)) {
          array_push($result->ids, $id);
          $problem_count++;
        } else
        if (empty($annieusermeta->lastname)) {
          array_push($result->ids, $id);
          $problem_count++;
        }
      }
