...
switch ($method) {
  ...
  case 'POST':
    if ($input) {
      //error_log("DEBUG: Initiate: input: ".json_encode($input));
      ...
      if ($areyouokay && $messagetemplate) {
        ...
        foreach ($destinations as $destination) {
          ...
          if (count($contactkeys)>0) {
            ...
            $areyouokay = false;
            error_log("FAILED: Initiate: action: FAILED to get key for: ".$destination);
          }
          ...
          if ($areyouokay) { //or key
            ...
            if ($areyouokay) {
              ...
              foreach($res["errors"] as $error) {
                error_log("ERROR: Initiate: " . $error["message"]);
                $areyouokay = false;
              }
              foreach($res["warnings"] as $warning) {
                error_log("WARNING: Initiate: " . $warning["message"]);
                $areyouokay = false;
              }
              //ok but may be FAILED cases
              // update message.status via response
              if ($areyouokay) {
                //error_log("DEBUG: Initiate: messageid=$messageid " . $res["messages"][$destination]["status"]);
