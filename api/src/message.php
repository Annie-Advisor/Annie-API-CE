...
switch ($method) {
  case 'GET':
    ...
    break;
  case 'PUT':
    //nb! only status will be updated!
    if ($key && $input) {
      $ret = $anniedb->updateMessage($key,$input);
      if ($ret !== false) {
        http_response_code(200);
  ...
  case 'POST':
    if ($input) {
      $ret = $anniedb->insertMessage($input);
      if ($ret !== false) {
        http_response_code(200);
        echo json_encode(array("status"=>"OK", "id"=>$ret));
      } else {
